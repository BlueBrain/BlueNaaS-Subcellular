...
class WSHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, msg):
        ...
        if cmd == "get_simulations":
            ...
            self.send_message("simulations", {"simulations": simulations}, cmdid=cmdid)
        ...
        if cmd == "revision_from_excel":
            self.send_message("revision_from_excel", revision_from_excel(msg["data"]), cmdid=cmdid)

        if cmd == "query_molecular_repo":
            query = msg["data"]
            result = db.query_molecular_repo(query)
            self.send_message("query_result", {"queryResult": result}, cmdid=cmdid)

        if cmd == "query_branch_names":
            search_str = msg["data"] if "data" in msg else ""
            branch_names = db.query_branch_names(search_str)
            self.send_message("branch_names", {"branches": branch_names}, cmdid=cmdid)

        if cmd == "get_user_branches":
            branches = db.get_user_branches(self.user_id)
            self.send_message("user_branches", {"userBranches": branches}, cmdid=cmdid)

        if cmd == "query_revisions":
            branch_name = msg["data"]
            revisions = db.query_revisions(branch_name)
            self.send_message("revisions", {"revisions": revisions}, cmdid=cmdid)
        ...
        if cmd == "get_revision":
            ...
            self.send_message("revision_data", {"revision": revision_data}, cmdid=cmdid)

        if cmd == "get_branch_latest_rev":
            branch = msg["data"]
            branch_latest_rev = db.get_branch_latest_rev(branch)
            self.send_message("branch_latest_rev", {"rev": branch_latest_rev}, cmdid=cmdid)

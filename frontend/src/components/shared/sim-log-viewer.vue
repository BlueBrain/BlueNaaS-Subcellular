<template>
  <div class="container">
    <div class="log-type-block">
      ...
      <i-select class="select ml-6" size="small" v-model="logType" @on-change="onLogTypeChange">
        <i-option v-for="logType in logTypes" :key="logType" :value="logType">
          {{ logTypeLabel[logType] }}
        </i-option>
      </i-select>

      <Checkbox class="ml-12" v-model="follow"> Follow logs </Checkbox>
    </div>

    <pre ref="log" class="log-content-block">{{ logContent }}</pre>
  </div>
</template>

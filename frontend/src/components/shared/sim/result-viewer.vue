...
<template>
  <div class="h-100 pos-relative">
    <div
      v-if="spatialViewerAvailable"
      class="ctrl"
    >
      <i-select
        v-model="currentViewKey"
        @on-change="onViewChange"
      >
        <i-option
          v-for="(viewLabel, viewKey) in view"
          :key="viewKey"
          :value="viewKey"
        >{{ viewLabel }}</i-option>
      </i-select>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.currentViewKey = this.spatialViewerAvailable
        ? 'allHorizontal'
        : 'temporal';
    ...
    computed: {
      ...
      spatialViewerAvailable() {
        return this.sim.solver === SimSolver.STEPS

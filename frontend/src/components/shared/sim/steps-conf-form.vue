<template>
  <i-form :label-width="100" @submit.native.prevent>
    ...
    <FormItem label="Stimulation">
      <Collapse class="small-collapse">
        <Panel>
          ...
          <div slot="content">
            <steps-stimulation-form v-model="conf.stimulation" @input="onChange" />
          </div>
        </Panel>
      </Collapse>
    </FormItem>
    <FormItem label="Spatial sampling">
      <Collapse class="small-collapse">
        <Panel>
          <span :class="{ 'grey-text': !conf.spatialSampling || !conf.spatialSampling.enabled }">
            {{ conf.spatialSampling && conf.spatialSampling.enabled ? 'On' : 'Off' }}
          </span>
          <div slot="content">
            <spatial-sampling-conf v-model="conf.spatialSampling" @input="onChange" />
          </div>
        </Panel>
      </Collapse>
    </FormItem>

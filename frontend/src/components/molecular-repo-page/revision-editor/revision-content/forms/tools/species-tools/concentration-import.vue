<template>
  <div>
    <Steps class="mb-24" :current="step">
      <Step title="Upload a file" icon="ios-cloud-upload" />
      <Step title="Configure" icon="ios-hammer" />
      <Step title="Import" icon="ios-git-merge" />
    </Steps>
    ...
    <transition name="fade-enter" mode="out-in">
      ...
      <div v-else-if="step === 1" key="configure" :step="1">
        ...
        <i-form :label-width="140">
          ...
          <FormItem label="Structures mapping">
            ...
            <Row :gutter="12" type="flex">
              <i-col span="10">
                <i-select v-model="structureMapping.name" placeholder="Select structure" filterable>
                  <i-option
          ...
          <FormItem label="Target concentration">
            <i-select v-model="config.concSource" filterable placeholder="Select concentration">
      ...
      <div v-else key="import">
        <i-button class="mb-24" @click="gotoConfiguration"> &lt; Back to configuration </i-button>
        ...
        >
          <template slot-scope="{ row, index }" slot="name">
            <span v-if="row.species.length === 1">
              {{ row.species[0].name }}
            </span>
            <RadioGroup v-else v-model="concImportCollection[index].specIdx" vertical>
              <Radio v-for="(spec, specIdx) in row.species" :key="spec.entityId" :label="specIdx">{{
                spec.name
              }}</Radio>
            </RadioGroup>
          </template>

          <template slot-scope="{ row }" slot="definition">
            <bngl-text
          ...
          <template slot-scope="{ row }" slot="current-conc">
            <bngl-text
              v-for="spec in row.species"
              :key="spec.entityId"
              class="block"
          ...
          <template slot-scope="{ row, index }" slot="new-conc">
            <span v-if="row.newConcentrations.length === 1">
              <bngl-text entity-type="parameter" :value="row.newConcentrations[0]" />
            </span>
            <RadioGroup v-else v-model="concImportCollection[index].newConcentrationIdx" vertical>
              <Radio v-for="(conc, concIdx) in row.newConcentrations" :key="conc" :label="concIdx">
                <bngl-text entity-type="parameter" :value="conc" />
              </Radio>
            </RadioGroup>
          </template>
        </i-table>

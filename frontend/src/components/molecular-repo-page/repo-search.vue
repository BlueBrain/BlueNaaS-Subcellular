<template>
  <div>
    <Card class="mt-12">
      <Row :gutter="24" type="flex">
        ...
        <i-col span="12">
          ...
          <i-select v-model="query.entityTypes" transfer multiple filterable>
            <i-option v-for="(label, entityType) in entityLabels" :key="entityType" :value="entityType">
              {{ label }}
            </i-option>
          </i-select>
        </i-col>
      ...
      <Row :gutter="24" class="mt-24">
        <i-col span="8">
          <Dropdown class="query-dropdown" @on-click="onFilterActionClick" transfer>
            <Button>
              Filter
        ...
        <i-col span="8" push="8">
          <i-button type="primary" long :disabled="!searchBtnEnabled" :loading="loading" @click="onSearchClick">
            Search
          </i-button>
        </i-col>
      </Row>
    ...
    <Card class="mt-24" v-if="repoQueryResult">
      <h3 v-if="queryResultEmpty">Search returned no results</h3>
      <search-content v-else />

      <Row class="mt-24" :gutter="12">
        <i-col span="8" offset="16">
          <revision-merge :disabled="queryResultEmpty" :versions="queryResultVersions" />
        </i-col>
      </Row>
    </Card>

    <Modal v-model="importFilterModalVisible" title="Import filter from a JSON file" class-name="vertical-center-modal">
      <file-import
      ...
      />
      <div slot="footer">
        <i-button @click="hideImportFilterModal"> Cancel </i-button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onFilterActionClick(action) {
      if (action === 'save') {
        const blob = new Blob([JSON.stringify(this.query)], {

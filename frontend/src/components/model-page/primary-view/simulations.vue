  ...
  computed: mapState({
    ...
    traceGraphBtnAvailable() {
      ...
      if (
        ...
        [SimStatus.STARTED, SimStatus.FINISHED, SimStatus.CANCELLED].includes(this.selectedSimulation.status)
      )
        return true

      if (this.selectedSimulation.solver === SimSolver.NFSIM && this.selectedSimulation.status === SimStatus.FINISHED)
        return true

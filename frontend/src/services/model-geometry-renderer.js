...
class ModelGeometryRenderer {
  ...
  initMolecules(moleculeNames) {
    ...
    this.moleculeConfig = moleculeNames.reduce(
      (acc, molName, idx) => ({ ...acc, [molName]: { color: molColors[idx], visible: true } }),
      {}
    )
  ...
  renderMolecules(spatialSample) {
    ...
    const processCount = (moleculeName, structureType, simplexIdx, count) => {
      ...
      const genRandPointFunc =
        structureType === 'compartment' ? this.genRandTetPoint.bind(this) : this.genRandTriPoint.bind(this)
  ...
  genRandTetPoint(tetIdx) {
    ...
    return [p0[0] + p1[0] + p2[0] + p3[0], p0[1] + p1[1] + p2[1] + p3[1], p0[2] + p1[2] + p2[2] + p3[2]]
  }
  ...
  setDisplayConf(displayConf) {
    ...
    this.modelMeshObject.traverse((obj) => {
      ...
      obj.material.opacity = obj.type === 'Mesh' ? conf.meshSurfaceOpacity : conf.meshWireframeOpacity
    })
